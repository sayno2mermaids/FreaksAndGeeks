(function (root, factory) {
	...
	module.labeledPie = function () {
		this.render = function (data, target, w, h, options) {
			...
			function elementRendering(data) {
	...
	module.line = function () {
		this.render = function (data, target, w, h, options) {
			...
			if (data.length > 0) {
				...
				// define the intial scale (range will be updated after we determine the final dimensions)
				var x = data[0].values[0][options.xValue] instanceof Date ? d3.time.scale() : d3.scale.linear();
	...
	module.zoomScatter = function () {
		this.render = function (data, target, w, h, options) {
			...
			if (data.length > 0) {
				...
				// define the intial scale (range will be updated after we determine the final dimensions)
				var xDomain = [d3.min(data, function (d) {
					return d3.min(d.values, function (d) {
						return d[options.xValue];
					});
				}), d3.max(data, function (d) {
		...
		this.render = function (data, target, width, height, options) {
			...
			cell.append("svg:rect")
				...
				.attr("data-placement", function (d) {
					var node = d3.select(d)[0][0];
					return (node.y > 0) ? "top" : "bottom";
				})
				.attr("data-html", true)
			...
			function zoom(d) {
				...
				var t = chart.selectAll("g.cell,.grouper").transition()
					.duration(750)
					.attr("transform", function (d) {
						return "translate(" + x(d.x) + "," + y(d.y) + ")";
					})
			...
			function applyGroupers(target) {
				...
				var groupers = chart.selectAll(".grouper")
					.data(top_nodes)
					.enter().append("svg:g")
					.attr("class", "grouper")
					.attr("transform", function (d) {
